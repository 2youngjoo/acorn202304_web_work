...
public class FortuneServlet extends HttpServlet{
   ...
   protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
       ...
       *  담은 데이터를 얻어내는 방법은
       *  HttpServletRequest 객체의 getAtrribute(key) 메소드를 활용하면 된다.
       *  단, 담을 때는 object type으로 받아주기 때문에 어떤 type 이든 담을 수 있지만
       *  얻어 낼때 역시 object type으로 return 되기 때문에 원래 type을 기억하고 있다가
       *  casting 하는 것이 필요하다
       *  
       *  예를 들어 "fortuneToday"라는 key 값으로 String type을 담았다면
       *  얻어낼 때는 아래와 같이 code가 된다.
       *  
